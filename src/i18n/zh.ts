...
export const messages: Record<string, string> = {
  ...
  'card.repo.note': '也可以回复 `/repo <编号>` 切换（如 `/repo 1`），或直接 `/repo <路径|项目名>`（如 `/repo botmux`、`/repo ~/projects/foo`）跳过本卡片；`/repo wt <编号|项目名> [分支名]` 基于主 checkout 新建 worktree 并打开；`/repo wt-batch <分支名> <编号|项目名|路径>...` 可批量创建但不自动切换。',
  'card.repo.placeholder_worktree': '🌿 选主仓库新建 worktree 打开',
  ...
  'cmd.repo.worktree_batch_usage': '用法：`/repo wt-batch <分支名> <编号|项目名|路径>...` — 至少指定 2 个仓库；批量创建同名分支 worktree，不自动切换会话。',
  'cmd.repo.worktree_batch_min_targets': '批量创建至少需要 2 个仓库；只有 1 个仓库时请使用 `/repo wt <编号|项目名|路径> <分支名>`。',
  'cmd.repo.worktree_batch_creating': '🌿 正在为 {count} 个仓库批量创建分支 `{branch}` 的 worktree（顺序执行，失败会继续）…',
  'cmd.repo.worktree_batch_summary': '🌿 批量 worktree 创建完成（分支 `{branch}`）：成功 {success}，失败 {failure}。会话工作目录未切换。',
  'cmd.repo.worktree_batch_success_heading': '✅ 成功：',
  'cmd.repo.worktree_batch_failure_heading': '❌ 失败：',
  ...
  'help.repo_wt_batch': '/repo wt-batch <分支> <编号|项目名|路径>... - 为多个仓库批量创建同分支 worktree（不自动切换）',
