...
export const messages: Record<string, string> = {
  ...
  'card.repo.note': 'You can also reply `/repo <N>` (e.g. `/repo 1`), or `/repo <path|name>` directly (e.g. `/repo botmux`, `/repo ~/projects/foo`) to skip this card. `/repo wt <N|name> [branch]` opens a fresh worktree from the main checkout. `/repo wt-batch <branch> <N|name|path>...` creates worktrees in batch without switching this session.',
  'card.repo.placeholder_worktree': '🌿 Open a main checkout as a new worktree',
  ...
  'cmd.repo.worktree_batch_usage': 'Usage: `/repo wt-batch <branch> <N|name|path>...` — specify at least 2 repos; creates same-branch worktrees in batch without switching this session.',
  'cmd.repo.worktree_batch_min_targets': 'Batch worktree creation needs at least 2 repos; for one repo use `/repo wt <N|name|path> <branch>`.',
  'cmd.repo.worktree_batch_creating': '🌿 Creating worktrees for branch `{branch}` across {count} repos (sequential; failures continue)…',
  'cmd.repo.worktree_batch_summary': '🌿 Batch worktree creation finished (branch `{branch}`): {success} succeeded, {failure} failed. Session working directory was not switched.',
  'cmd.repo.worktree_batch_success_heading': '✅ Successes:',
  'cmd.repo.worktree_batch_failure_heading': '❌ Failures:',
  ...
  'help.repo_wt_batch': '/repo wt-batch <branch> <N|name|path>... - Create same-branch worktrees for multiple repos without switching',
